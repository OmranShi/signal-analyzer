...
def print_header():
    ...
    print(Fore.CYAN + f"   Simulated UART @ {BAUD_RATE} baud  |  Sample Rate: {SAMPLE_RATE_HZ} Hz")
    print(Fore.CYAN + "=" * 65)
...
def print_sample(sample_id, ts, cpu, ram, voltage, current, anomalies):
    ...
    v_color = Fore.RED if voltage < VOLTAGE_FAULT else (Fore.YELLOW if voltage < VOLTAGE_MIN else Fore.GREEN)
    print(f"  Voltage    : {v_color}{voltage}V{Style.RESET_ALL}  (nominal: 3.3V)")

    i_color = Fore.RED if current > 160 else (Fore.YELLOW if current > 140 else Fore.GREEN)
...
def main():
    ...
    try:
        ...
        logging.info(f"=== Session Ended | {sample_id} samples | {total_anomalies} anomalies ===")
